...
public class HotelReservationSystem {
	...
	public int calculateCostForRewardee(String checkIn, String checkOut, boolean reward) {
		LocalDate checkInDate = convertStringToDate(checkIn);
		LocalDate checkOutDate = convertStringToDate(checkOut);
		addDetail("Lakewood", 110, 90, 3, 80, 80);
		addDetail("Bridgewood", 150, 50, 4, 110, 60);
		...
		for (HotelDetails hotelDetail : hotelDetails) {
			...
			if (hotelRent <= minRate) {
				minRate = hotelRent;
				cheapestHotelName.add(hotelDetail.getName());
				System.out.println("Cost : " +minRate+", Name : "+hotelDetail.getName());
			}
		}
		return minRate;
	}

	public ArrayList<String> nameListForRewardee(String checkIn, String checkOut, boolean reward) {
		LocalDate checkInDate = convertStringToDate(checkIn);
		LocalDate checkOutDate = convertStringToDate(checkOut);
		addDetail("Lakewood", 110, 90, 3, 80, 80);
		addDetail("Bridgewood", 150, 50, 4, 110, 60);
	...
	public String calculateCheapestRatedHotelForRewardCustomer(String checkIn, String checkOut, boolean reward) {
		nameListForRewardee(checkIn, checkOut, reward);
		Map.Entry<String, Integer> cheapestBestRatedHotel = null;
	...
	public static void main(String[] args) throws InvalidEntriesException {
		...
		System.out.println(
				"Enter choice \n1.FindCheapestHotelForRewardee \n2.cheapestRatedHotelForRewardCustomer \n3.Exit ");
		int entry = sc.nextInt();
			switch (entry) {
			case 1:
				obj.calculateCostForRewardee(checkIn, checkOut, reward);
				break;
			case 2:
				obj.calculateCheapestRatedHotelForRewardCustomer(checkIn, checkOut, reward);
				break;
			default:
